...
function ProductList() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await fetch(`http://localhost:3055/api/v1/product?page=${currentpage}`);
        if (!response.ok) {
    ...
    <>
      ...
      <div className="product">
        {searchResults.length > 0 ? (
          ...
          <>
            {
              searchResults.map((item) => <ProductItem key={item.id} item={item} />)
            }
            <div className="product__trans">
            <button className="product__prev" onClick={handlePrev}>Prev</button>
            <div className="product__page">{currentpage}</div>
            <button className="product__next" onClick={handleNext}>Next</button>
            </div>
          </>
        ) : (
           <>
            {allProducts.map((item) => <ProductItem key={item.id} item={item} />)}
            <div className="product__trans">
            <button className="product__prev" onClick={handlePrev}>Prev</button>
            <div className="product__page">{currentpage}</div>
            <button className="product__next" onClick={handleNext}>Next</button>
